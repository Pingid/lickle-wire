...
Most server platforms — Bun's `.ws()`, `ws`'s `WebSocketServer`, Deno's `upgradeWebSocket` — hand you the
same shape: `open`, `message`, `close` callbacks over something with a `send` and a `close`. `socketSource`
is `defineSource` and `defineLink` already wired for exactly that, so the platform glue is only the calls
into it:

```ts
import { socketSource } from '@lickle/wire/adapter'
...
`open`/`message`/`close`/`error` may fire before anything is serving `sockets.source`, or after it stopped
— both read as "nobody is listening" rather than a special case, because they go through the same
subscribe/unsubscribe `defineSource` already does for every other source.
