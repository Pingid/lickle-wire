...
import { accept, asLink, defineLink, defineSource, socketSource, type Link } from './adapter/index.ts'
...
/** Any platform whose websocket API is open/message/close callbacks over a socket. */
const socketSourceSample = () => {
  const sockets = socketSource<WebSocket, Envelope>()

  const wss: { on(event: 'connection', fn: (ws: WebSocket) => void): void } = {} as any
  wss.on('connection', (ws) => {
    sockets.open(ws)
    ws.addEventListener('message', (e) => sockets.message(ws, e.data))
    ws.addEventListener('close', () => sockets.close(ws))
    ws.addEventListener('error', (e) => sockets.error(ws, e))
  })
