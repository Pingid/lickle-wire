...
export interface Slot<T> extends Link<T> {
  /**
   ...
   */
  use(inner: Link<T>): Unsub
  /** The transport in use, or null. */
  readonly inner: Link<T> | null
}
...
export declare namespace Slot {
  export interface Options<T = unknown> {
     ...
     */
    onInner?: ((inner: Link<T> | null) => void) | undefined
...
export const slot = <T>(opts: Slot.Options<T> = {}): Slot<T> => {
  const own = opts.own ?? false
  const held = outbox<T>({ ...opts, onDrop: opts.onDrop })
  ...
  const announce = (next: Link<T> | null) => attempt(() => opts.onInner?.(next), undefined, opts.onError)
  ...
  const attach = (next: Link<T>): number => {
    const token = ++epoch
    inner = next
    stops = detacher()
    ...
    stops.add(next.listen((msg) => token === epoch && host.deliver(msg)))
    stops.add(
      next.changed((up) => {
        if (token !== epoch) return
        host.signal(up)
        if (up) held.drain((msg) => next.send(msg))
      }),
    )
  ...
  const link = defineLink<T>(
    (h) => {
      host = h
      return {
        send: (msg) => (inner && inner.up ? inner.send(msg) : held.hold(msg)),
        close: () => release(true),
